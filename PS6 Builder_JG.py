...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/joelleg/Desktop/trialloops-girgisjo/PS5 Builder_JG.py',
...
TY_message = visual.TextStim(win=win, name='TY_message',
...
for thisPhrase in phrase:
    ...
    TrialComponents = [key_resp, text_feedback, text_phrase, Prompt]
    ...
    while continueRoutine:
        ...
        if Prompt.status == STARTED:  # only update if drawing
            Prompt.setText('Re-type this phrase below:', log=False)
    ...
    phrase.addData('Prompt.started', Prompt.tStartRefresh)
    phrase.addData('Prompt.stopped', Prompt.tStopRefresh)
...
# update component parameters for each repeat
# keep track of which components have finished
Thank_youComponents = [TY_message]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *TY_message* updates
    if TY_message.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        TY_message.frameNStart = frameN  # exact frame index
        TY_message.tStart = t  # local t and not account for scr refresh
        TY_message.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(TY_message, 'tStartRefresh')  # time at next scr refresh
        TY_message.setAutoDraw(True)
    if TY_message.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > TY_message.tStartRefresh + 1.0-frameTolerance:
            # keep track of stop time/frame for later
            TY_message.tStop = t  # not accounting for scr refresh
            TY_message.frameNStop = frameN  # exact frame index
            win.timeOnFlip(TY_message, 'tStopRefresh')  # time at next scr refresh
            TY_message.setAutoDraw(False)
...
for thisComponent in Thank_youComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('TY_message.started', TY_message.tStartRefresh)
thisExp.addData('TY_message.stopped', TY_message.tStopRefresh)
